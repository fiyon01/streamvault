...
import { useCartoonFilterStore } from '@/store/filter-store';
import { discoverCartoons } from '@/app/actions/discover';
import { ContentCard } from '@/components/ui/content-card';
import {
  SlidersHorizontal, X, RotateCcw, Film, Tv, Zap,
  Check, ChevronDown, ChevronUp, Loader2, Search, Laugh
} from 'lucide-react';
import { cn } from '@/lib/utils/cn';

const GENRES = [
  { label: 'Action', id: 'action' }, { label: 'Comedy', id: 'comedy' },
...
export default function CartoonsDiscoverPage() {
  const filters = useCartoonFilterStore();
  ...
  const runSearch = useCallback(() => {
    startTransition(async () => {
      const data = await discoverCartoons(filters);
      setResults(data);
    });
  }, [
    filters.contentType, filters.targetDemo, filters.animationStyle, 
    filters.minRating, filters.minVoteCount, filters.selectedGenres,
    filters.excludeGenres, filters.sortBy, filters.yearFrom, filters.yearTo,
  ]);
  ...
  const activeFilterCount = [
    ...
    filters.targetDemo !== 'all',
    filters.animationStyle !== 'all',
    filters.minRating > 0,
  ...
  return (
    <div className="min-h-screen bg-[#05050f] text-white flex flex-col">
      ...
      <div className="relative overflow-hidden border-b border-white/[0.06]">
        <div className="absolute inset-0 bg-gradient-to-br from-[#f97316]/8 via-transparent to-[#eab308]/5" />
        <div className="absolute top-0 left-1/4 w-[500px] h-[200px] bg-[#f97316]/10 rounded-full blur-[80px] -translate-y-1/2" />
        <div className="relative px-6 md:px-10 py-10 max-w-screen-2xl mx-auto w-full">
          <div className="flex items-end justify-between gap-6">
            <div>
              <div className="flex items-center gap-3 mb-3">
                <div className="w-8 h-8 rounded-lg bg-[#f97316]/20 border border-[#f97316]/30 flex items-center justify-center">
                  <Laugh size={16} className="text-[#f97316]" />
                </div>
                <span className="text-[#f97316] text-xs font-bold uppercase tracking-[0.2em]">Find your next animated obsession</span>
              </div>
              <h1 className="text-4xl md:text-5xl font-black tracking-tight text-white">Cartoons & Animation</h1>
            ...
            <div className="flex items-center gap-3 shrink-0">
              ...
              >
                ...
                Filters
                {activeFilterCount > 0 && (
                  <span className="w-5 h-5 rounded-full bg-[#f97316] text-white text-[10px] font-black flex items-center justify-center">
                    {activeFilterCount}
                  </span>
                )}
              </button>
          ...
          {activeFilterCount > 0 && (
            <div className="flex flex-wrap gap-2 mt-5 pt-5 border-t border-white/[0.06]">
              {filters.contentType !== 'both' && (
                <Chip theme="cartoon" label={filters.contentType === 'movie' ? '🎬 Movies' : '📺 TV shows'} onRemove={() => filters.setFilter('contentType', 'both')} />
              )}
              {filters.targetDemo !== 'all' && (
                <Chip theme="cartoon" label={filters.targetDemo === 'adult' ? 'Adult Animation' : 'Kids & Family'} onRemove={() => filters.setFilter('targetDemo', 'all')} />
              )}
              {filters.animationStyle !== 'all' && (
                <Chip theme="cartoon" label={filters.animationStyle} onRemove={() => filters.setFilter('animationStyle', 'all')} />
              )}
              {filters.minRating > 0 && (
                <Chip theme="cartoon" label={`⭐ ${filters.minRating}+ Rating`} onRemove={() => filters.setFilter('minRating', 0)} />
              )}
              {filters.minVoteCount > 0 && (
                <Chip theme="cartoon" label={`🗳 ${filters.minVoteCount.toLocaleString()}+ Votes`} onRemove={() => filters.setFilter('minVoteCount', 0)} />
              )}
              {filters.yearFrom > 0 && (
                <Chip theme="cartoon" label={`From ${filters.yearFrom}`} onRemove={() => filters.setFilter('yearFrom', 0)} />
              )}
              {filters.yearTo > 0 && (
                <Chip theme="cartoon" label={`To ${filters.yearTo}`} onRemove={() => filters.setFilter('yearTo', 0)} />
              )}
              {filters.selectedGenres.map(g => (
                <Chip theme="cartoon" key={g} label={g} onRemove={() => filters.setFilter('selectedGenres', filters.selectedGenres.filter(x => x !== g))} />
              ))}
              {filters.excludeGenres.map(g => (
                <Chip theme="cartoon" key={g} label={`Not: ${g}`} onRemove={() => filters.setFilter('excludeGenres', filters.excludeGenres.filter(x => x !== g))} />
              ))}
            </div>
          )}
        </div>
      ...
      <div className="flex flex-1 max-w-screen-2xl mx-auto w-full">
        ...
        <aside className={cn(
          ...
          <div className="flex-1 lg:flex-none p-6 space-y-8 lg:sticky lg:top-0 max-h-screen overflow-y-auto scrollbar-none pb-24 lg:pb-6">
            ...
            {/* Target Demo */}
            <FilterSection title="Target Demo">
              <div className="flex flex-col gap-2">
                {[
                  { value: 'all', label: 'All' },
                  { value: 'adult', label: 'Adult Animation' },
                  { value: 'kids', label: 'Kids & Family' },
                ].map(({ value, label }) => (
                  <button
                    key={value}
                    onClick={() => filters.setFilter('targetDemo', value as any)}
                    className={cn(
                      "py-2 px-3 rounded-lg border text-xs font-bold transition-all text-left",
                      filters.targetDemo === value
                        ? "bg-[#f97316]/15 border-[#f97316]/40 text-[#f97316]"
                        : "bg-white/[0.03] border-white/[0.06] text-white/40 hover:text-white/70 hover:bg-white/[0.06]"
                    )}
                  >
            ...
            {/* Animation Style */}
            <FilterSection title="Animation Style">
              <div className="flex flex-col gap-2">
                {[
                  { value: 'all', label: 'All' },
                  { value: '2d', label: '2D / Traditional' },
                  { value: '3d', label: '3D / CGI' },
                  { value: 'stop-motion', label: 'Stop-Motion' },
                  { value: 'anime-western', label: 'Anime-Inspired Western' },
                ].map(({ value, label }) => (
                  <button
                    key={value}
                    onClick={() => filters.setFilter('animationStyle', value as any)}
                    className={cn(
                      "py-2 px-3 rounded-lg border text-xs font-bold transition-all text-left",
                      filters.animationStyle === value
                        ? "bg-[#f97316]/15 border-[#f97316]/40 text-[#f97316]"
                        : "bg-white/[0.03] border-white/[0.06] text-white/40 hover:text-white/70 hover:bg-white/[0.06]"
                    )}
                  >
            ...
            {/* Min Rating */}
            <FilterSection title={`Min Rating  ${filters.minRating > 0 ? `— ⭐ ${filters.minRating}+` : ''}`}>
              <div className="flex flex-wrap gap-2">
                {RATING_STEPS.map(r => (
                  <button
                    key={r}
                    onClick={() => filters.setFilter('minRating', r)}
                    className={cn(
            ...
            {/* Min Vote Count */}
            <FilterSection title={`Min Votes${filters.minVoteCount > 0 ? ` — ${filters.minVoteCount.toLocaleString()}+` : ''}`}>
              <div className="flex flex-wrap gap-2">
                {[
                  { label: 'Any', value: 0 },
                  { label: '100+', value: 100 },
                  ...
                  <button key={o.value} onClick={() => filters.setFilter('minVoteCount', o.value)}
                    className={cn('px-3 py-1.5 rounded-lg text-xs font-bold border transition-all',
                      filters.minVoteCount === o.value
                        ? 'bg-[#f97316]/20 border-[#f97316]/40 text-[#f97316]'
                        : 'bg-white/[0.03] border-white/[0.06] text-white/40 hover:text-white/70'
                    )}>
                    {o.label}
            ...
            {/* Year Range */}
            <FilterSection title={`Release Year${filters.yearFrom > 0 || filters.yearTo > 0 ? ' — active' : ''}`}>
              <div className="flex gap-2 items-center">
                <select value={filters.yearFrom || ''} onChange={e => filters.setFilter('yearFrom', Number(e.target.value) || 0)}
                  className="flex-1 bg-white/5 border border-white/10 text-white text-xs rounded-lg px-2 py-2 focus:outline-none focus:border-[#f97316]/50">
            ...
            {/* Exclude Genres */}
            {filters.selectedGenres.length > 0 && (
              <FilterSection title="Exclude Genres">
                <div className="flex flex-wrap gap-2">
          ...
          {isLoading ? (
            ...
            <div className="flex flex-col items-center justify-center min-h-[60vh] text-center">
              ...
              <button onClick={() => filters.resetFilters()}
                className="mt-8 px-6 py-3 bg-[#f97316]/15 hover:bg-[#f97316]/25 border border-[#f97316]/30 text-[#f97316] rounded-xl font-bold text-sm transition-all">
                Reset Filters
              </button>
            </div>
          )}
