...
import { ArrowLeft, Calendar, Play, Star, Tv } from 'lucide-react';
import { KdramaPlayer } from '@/components/kdrama/kdrama-player';
import { KdramaRail } from '@/components/kdrama/kdrama-card';
import { TrailerButton } from '@/components/ui/trailer-button';
import { tmdb } from '@/lib/tmdb/api';

type KdramaDetailPageProps = {
  params: {
    id: string;
  };
};

export async function generateMetadata({ params }: KdramaDetailPageProps) {
  const { id } = await params;
  try {
    const details = await tmdb.getDetails('tv', id);
    return {
      title: `${details.name || 'K-Drama'} | StreamVault`,
    };
  } catch {
    return { title: 'K-Drama | StreamVault' };
  }
}

export default async function KdramaDetailPage({ params }: KdramaDetailPageProps) {
  const { id } = await params;
  ...
  try {
    ...
    console.error('K-drama details failed', error);
    notFound();
  }

  const title = details.name || details.title || 'K-Drama';
  ...
  return (
    <div className="min-h-full bg-bg">
      <section className="relative overflow-hidden border-b border-border">
        ...
        <div className="relative z-10 grid gap-8 px-6 py-10 md:px-12 lg:grid-cols-[220px_minmax(0,1fr)] lg:py-14">
          ...
          <div className="flex flex-col justify-end">
            <Link href="/kdrama" className="mb-8 inline-flex w-fit items-center gap-2 text-sm font-bold text-muted hover:text-text">
              <ArrowLeft size={16} />
              K-Drama Hub
            </Link>
            ...
            <div className="mt-7 flex flex-wrap items-center gap-3">
              ...
              <Link
                href={`/discover?vertical=kdrama&contentType=tv&originCountry=KR&originalLanguage=ko&selectedGenres=Drama&sortBy=rating`}
      ...
      <div className="space-y-12 px-6 py-8 md:px-12">
        ...
        <KdramaRail title="More Korean Dramas" items={similar} />
      </div>
    </div>
  );
}
