...
export function PinoyCard({ id, title, posterPath, year, rating, label = 'Pinoy drama' }: PinoyCardProps) {
  const imageSrc = posterPath ? `https://image.tmdb.org/t/p/w500${posterPath}` : null;

  return (
    <Link href={`/pinoy/${id}`} className="group block">
      <div className="overflow-hidden rounded-xl bg-surface transition duration-300 group-hover:-translate-y-1 group-hover:shadow-2xl group-hover:shadow-black/50">
        ...
        <div className="p-3">
          ...
          <p className="mt-1 text-xs text-muted">{year || label}</p>
        </div>
      </div>
    </Link>
  );
...
export function PinoyRail({ title, items, label }: { title: string; items: any[]; label?: string }) {
  ...
  return (
    <section className="space-y-4">
      <div className="flex items-end justify-between gap-4">
        ...
        <span className="text-xs font-bold uppercase tracking-[0.16em] text-muted">Pinoy TV</span>
      </div>
